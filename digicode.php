				...
				<?php
					...
					elseif ($_SESSION['level'] == 'user') {
							...
							$req=$cnx->prepare("SELECT * 
								FROM mrbs_entry,mrbs_entry_digicode 
							...
							<?php while($resultat) { 
								$id = $resultat->id;
								$roomid = $resultat->room_id;

								$req=$cnx->prepare("SELECT *
									FROM mrbs_entry_digicode
									WHERE id=:id");
								$req->bindValue(":id",$id);
							?>
							<?php if(!($date->getTimestamp() > $resultat->end_time) && ($date->getTimestamp() + 86400 >= $resultat->start_time)){ ?>
							<tr>		
								<td scope="row">
								<?=$resultat->room_id;?>
								</td>
